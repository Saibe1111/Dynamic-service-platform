...
public class ServiceBRiProgrammeur extends ServiceBRi {
	...
	private Programmeur connexionInscription(BufferedReader in, PrintWriter out) throws IOException {
		...
		switch (choix) {
			...
			case 2 -> {
				...
				if (p == null) {
					out.println("Impossible de s'inscrire. Login dÈj‡† existant");
					System.err.println("Impossible d'inscrire. Login dÈj‡† existant");
					return null;
				}
				break;
			}
			default -> {
				out.println("NumÈro incorrect !!");
				System.err.println("NumÈro incorrect !!");
				return null;
			}
		}
		return p;
	...
	private boolean choixAction(BufferedReader in, PrintWriter out, Programmeur p) throws IOException {
		...
		int choix = 0;
		out.println(
				"1 - Fournir un nouveau service##2 - Mettre-√†-jour un service##3 - Changer l'adresse du serveur ftp##4 - Quitter");
		try {
			choix = Integer.parseInt(in.readLine());
		} catch (Exception e) {
			out.println("Num√©ro incorrect !!");
			System.err.println("Impossible de convertir la saisie utilisateur en un int !");
			return false;
		}
		...
		switch (choix) {
			...
			case 2 -> {
				...
				out.println("Veuillez donner le nom de votre service √† mettre √† jour");
				String service = in.readLine();
			...
			default -> {
				out.println("Num√©ro incorrect !!");
				System.err.println("Num√©ro incorrect !!");
				return false;
			}
		}
